...
- We will first need to export the `shinyngs_app` directory to our PCs. You can do this with your FTP apps (FileZilla or MobaXterm), or by opening a new terminal window:

```
#move to directory where you want the download to go
cd /Users/Alex/Downloads

#copy directory from HAWK
scp -r c.c1234567@hawklogin.cf.ac.uk:/scratch/c.c1234567/rnaseq/output/shinyngs_app .
```

- Now we can open the shinyngs_app directory and open the subdirectory. This should be the same name as we used in the parameter file.
- Within this directory, we have `app.R` and `data.rds` .
- The pipeline kindly takes all of the produced data, formats it, and saves it as an R data object. This can then be read by the app using the script.
- To get the app running, open RStudio. Then click `file > open project` and navigate to the directory where the `app.R` and `data.rds` are located.
- You will now see that the bottom-right panel is populated with those files:

<img src="/assets/img/figure-28.png" alt="Opening shiny app project on RStudio" width="1000"/>

- You will notice that there are 2 extra files added to the directory, these are generated when we 'create' a new project in R. We can ignore these.
- We now need to open the app.R script which will allow us to run the app. To do this, click on the `app.R` in the bottom-right panel. This will open a new panel on the top-left with the script.

<img src="/assets/img/figure-29.png" alt="Opening shiny app script in RStudio" width="1000"/>

- **If we have correctly followed the Pre-course materials on how to install R, RStudio and the relevant packages, we should get no problems on this next part**
- Now that we have everything open, we can click on the `Run App` button at the top-right of the script panel, which will run the app and open a new screen:

<img src="/assets/img/figure-30.png" alt="Shiny app window" width="1000"/>

- You can see that there are various tabs along the top of the app. Clicking on each of these will load a new page with the dedicated results etc. Lets go throught them.

**Note: Each section contains a help button at the top-right containing a description of the analysis and references for the tools etc.**

**Sample Data**
- Clicking on this tab opens a drop-down menu with `Experimental Data` and `Annotation`.
- `Annotation` section contains the full list of the genes in your dataset with all the corresponding gene information such as gene length, chromosome location, and gene type.
- The `Experimental Data` section contains a table that is identical to the sample sheet that we provided the pipeline. It gives us some information about the samples, their fastq locations, group names etc.

**QC/exploratory**
- This tab contains the following sections: Distribution plots, PCA, PCA vs Experiment, Clustering dendrogram, Clustering heatmap, and Feature-wise clustering.

**Distribution plots**
- This section shows us the distributions of our count data. Theres not much we can take from this data, but we do have options along the left-hand side that we can change to edit the graph.

**PCA**
- This section shows us the PCA plot(s) for our data. There are 2 sections: Components and Loadings.
- The Components section shows each samples location on the PCA plot, whereas the Loadings section shows the locations of the genes on the PCA plot.
- Below the PCA plot, you will find a table containing each sample/gene and its respective PC loading.
- Along the left side of the page, we can also see options that we can edit and/or change which will change how the data is presented to us.
- As aforementioned, we tend to use this data to show how similar/different samples are in respect to each other. It is up to you whether you want to deep dive into this data or not.

**PCA vs Experiment**
- This section contains a heatmap that shows the PC's relationship to the experimental structure.
- I don't use these plots, as I tend to only have one comparison that I am making during my analyses, but if you have more than two groups and have multiple conditon columns, then this analysis may be beneficial to you. 
- For a detailed description of this analysis, click the help button on the top-right of the window.

**Clustering Dendrogram**
- This section shows a dendrogram for your data. It is performed on the top 1000 genes that contain the most variance in the dataset.
- Again, along the left side we have options to edit this plot.

**Clustering Heatmap**
- This section contains a heatmap that shows the correlation between the samples.
- This plot is another way of seeing how similar/different our samples are.

**Feature-wise Clustering**
- This section contains plots that show expression of clusters in our data.
- The analysis attempts to create clusters of genes that behave similarly within our groups, and differently between the groups.
- As we can see, cluster one (top left plot) is 'over expressed' in the first three samples, but 'under expressed in the other three.
- Below the plots is a table containing the genes that belong to these clusters. 
- This section is a handy way of quickly spotting differences between the groups and potentially identifying key players in expression.

<img src="/assets/img/figure-31.png" alt="feature-wise-clustering-section" width="1000"/>
